...
class DinoVisionTransformer(nn.Module):
    def __init__(
        ...
        **ignored_kwargs,
        ...
        super().__init__()
        if "init_values" in ignored_kwargs and layerscale_init is None:
            layerscale_init = ignored_kwargs.pop("init_values")
        else:
            ignored_kwargs.pop("init_values", None)
        ignored_kwargs.pop("block_chunks", None)
        ignored_kwargs.pop("use_flash2_dino", None)
        ignored_kwargs.pop("softmax_scale", None)
        ignored_kwargs.pop("train_avg_length", None)
    ...
    def _norm_patch_tokens(self, x: Tensor) -> Tensor:
        if self.untie_cls_and_patch_norms or self.untie_global_and_local_cls_norm:
            return self.norm(x[:, self.n_storage_tokens + 1 :])
        return self.norm(x)[:, self.n_storage_tokens + 1 :]

    def forward_interval_features(self, x: Tensor, masks: Optional[Tensor] = None) -> List[Tensor]:
        if self.dino_layer_idxs is not None:
            return self.forward_features_with_idxs(x, masks)
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if (i + 1) % interval == 0 and i != len(self.blocks) - 1:
                features.append(x[:, self.n_storage_tokens + 1 :])
        features.append(self._norm_patch_tokens(x))
        return features

    def forward_features_with_idxs(self, x: Tensor, masks: Optional[Tensor] = None) -> List[Tensor]:
        x, (H, W) = self.prepare_tokens_with_masks(x, masks)
        features = []
        for i, blk in enumerate(self.blocks):
            rope_sincos = self.rope_embed(H=H, W=W) if self.rope_embed is not None else None
...
def extract_3_7_11_features(
    ...
    *,
    ...
    layer_indices = [2, 6, 10] if one_based else [3, 7, 11]
